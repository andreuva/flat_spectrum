class ESE:
    ...
    def __init__(self, nu_quad_w, B):
        """ 
            nu_quad: array of the frequency quadrature weights
            B: object of the magnetic field vector with .x, .y, .z components (in gauss)
            return value: None
        """
        pass
    
    def addStokes(self, stokes, ray, ray_w):
        """ Called per every Lambda iteration, grid point, and ray direction.
            stokes: list of 4 arrays of Stokes parameters [I,Q,U,V] in given point and direction (all frequencies)
            ray: object with .theta and .chi variables defining the ray of propagation direction
